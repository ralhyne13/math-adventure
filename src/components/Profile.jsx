    ...
    <Modal title={modalTitle} onClose={onClose} presentation={presentation}>
      ...
      {profileTab === "stats" && (
        <div className="panelSectionStack">
          <div className="toast panelCard" style={{ marginTop: 0 }}>
            <div>
              <strong>Global</strong>
              <div className="sub" style={{ marginTop: 8 }}>
                Joueur : <b>{authUser.pseudoDisplay}</b>
                <br />
                Niveau : <b>{level}</b> | Pièces : <b>{coins}</b> | Diamants : <b>{cosmeticDust}</b> | Serie de connexion : <b>{loginStreak}/7</b>
                <br />
          ...
          <div className="small panelSectionLabel" style={{ marginTop: 12 }}>
            Records par classe vers difficulte vers mode (meilleur score de session) :
          </div>

          <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
            {GRADES.map((g) => (
              <div key={g.id} className="shopCard panelCard">
      ...
      {profileTab === "album" && (
        <div className="panelSectionStack">
          ...
          <div className="small panelSectionLabel" style={{ marginTop: 12 }}>Skins</div>
          <div className="shopGrid" style={{ marginTop: 10 }}>
            {SKINS.map((s) => {
              const owned = ownedSkins.includes(s.id);
              const equipped = skinId === s.id;
              return (
                <div key={s.id} className={`shopCard panelCard ${owned ? "" : "badgeLocked"}`}>
          ...
          <div className="small panelSectionLabel" style={{ marginTop: 12 }}>Avatars</div>
          <div className="shopGrid" style={{ marginTop: 10 }}>
            {AVATARS.map((a) => {
              const owned = ownedAvatars.includes(a.id);
              const equipped = avatarId === a.id;
              return (
                <div key={a.id} className={`shopCard panelCard ${owned ? "" : "badgeLocked"}`}>
          ...
          <div className="small panelSectionLabel" style={{ marginTop: 12 }}>Effets</div>
          <div className="shopGrid" style={{ marginTop: 10 }}>
            {answerEffects.map((fx) => {
              ...
              return (
                <div key={fx.id} className={`shopCard panelCard ${owned ? "" : "badgeLocked"}`}>
